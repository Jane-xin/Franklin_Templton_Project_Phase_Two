        ...
        # ───── INVESTMENT VALUATION (FAIR VALUE HIERARCHY) ─────
         investment_level_1: Optional[int] = Field(
            description=(
                ...
                "Note: Values in parentheses (e.g., (3,705)) represent negative cash flows and should be treated as such."              "Note: Values in parentheses (e.g., (3,705)) represent negative cash flows and should be treated as such."
                " If a single 'Level 1 total' is shown for the {eoy_date} column, use that value. "
                " If no single 'Level 1 total' is displayed, sum all Level 1 ASSET line items in the {eoy_date} column to obtain the total (exclude LIABILITIES). "
                " Include 'deposits' (e.g., money market funds) only when they are measured at fair value and explicitly classified as Level 1 in the hierarchy; "
                " exclude bank deposits carried at cost or amortized cost. "
                " If the Note presents 'Total investments at fair value' and a separate 'Total short-term investments' for Level 1, add the Level 1 amounts from both sections; "
                " however, do not double count short-term items if the disclosure indicates they are already included in 'investments at fair value'—prefer a single Level 1 total when provided. "
                " If multiple fiscal years are shown, use the column that aligns with {eoy_date}."
            )
        )
