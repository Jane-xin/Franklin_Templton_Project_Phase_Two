...
class StatementOfCashFlows2024(BaseModel):
    ...
    total_non_cash_exp: Optional[float] = Field(
        description=(
            ...
            "If not, calculate this as the sum of all clearly labeled non-cash expense items within the operating activities section of the cash flow statement. "
            "These may include (but are not limited to): depreciation, amortization, loss on disposal of assets, unrealized (gain)/loss on investments, non-cash interest, bad debt expense, stock-based compensation, and asset impairments. "
            "Do not include working capital adjustments, financing or investing non-cash items, or other non-operating accruals. "
    ...
    change_in_working_capital: Optional[float] = Field(
        description=(
            ...
            "Only extract this value from the detailed 'Cash Flows from Operating Activities' section, specifically from the subsection "
            "titled 'Change in assets and liabilities', 'Changes in operating assets and liabilities', 'changes in' or any equivalent label. "
            "If a line labeled exactly as 'Change in Working Capital' is available, extract that directly. "
            "Otherwise, calculate it by summing **all individual line items** listed under this subsection. These typically include:\n"
